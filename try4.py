...
def netlist_to_xml(components, commands, subcircuits):
    ...
    for comp in components:
        ...
        # [MODIFIED] Add scale attribute from default_scaling_ratios
        comp_elem.set("scale", str(default_scaling_ratios.get(comp_type, 1.0)))
...
def update_xml_positions_and_directions(xml_root, pos, directions, flips=None, scaling_ratios=None):
    ...
    for comp in components_elem:
        ...
        if cid in scaling_ratios:
            comp.set("scale", str(scaling_ratios[cid]))
            print(f"[DEBUG] Set scale for {cid}: {scaling_ratios[cid]}")
...
def get_component_bbox(component_type,
                       ...
                       scale=1.0,  # [MODIFIED] Add scale parameter
                       **kwargs):
    print(
        f"[DEBUG] Getting bounding box for component type '{component_type}' at position {pos} with direction '{direction}', flip '{flip}', and scale '{scale}'")
    ...
    # Apply scaling
    element.scale = scale  # [MODIFIED] Apply per-component scale
    print(f"[DEBUG] Applied scale {element.scale} to component.")
    ...
    if component_type in ['GND', 'ground']:
        ymax_global -= shrink_size * scale
    elif component_type in ['R', 'C', 'L', 'D', 'V', 'I', 'E', 'H', 'F', 'G', 'S']:
        xmin_global += shrink_size * scale
        xmax_global -= shrink_size * scale
    elif component_type == 'J':
        xmax_global -= shrink_size * scale
        ymin_global += shrink_size * scale
        ymax_global -= shrink_size * scale
        # Prevent wire crossing the junction field-effect transistor.
        xmin_global -= shrink_size * scale * 2
    elif component_type == 'Q':
        xmin_global += shrink_size * scale
        ymin_global += shrink_size * scale
        ymax_global -= shrink_size * scale
        # Prevent wire crossing the bipolar transistor.
        xmax_global += shrink_size * scale * 2
    elif component_type == 'M':
        xmin_global += shrink_size * scale
        ymin_global += shrink_size * scale
        ymax_global -= shrink_size * scale
        xmax_global -= shrink_size * scale  # Changed from += to -= to match user's configuration
...
def draw_component_or_node(d, elements, pins, node, node_info, x, y, direction, component_boxes, G, flip='none', scaling_ratios=None):
    ...
    try:
        if ctype_for_bbox == 'M':
            ...
            # [MODIFIED] Retrieve scale from scaling_ratios
            scale = scaling_ratios.get(node, default_scaling_ratios.get(ctype_for_bbox, 1.0))
            bbox, anchors_local, anchors_global = get_component_bbox(
                ...
                scale=scale,  # Pass per-component scale
    ...
    if ctype_for_bbox == 'M':
        ...
        if model.startswith('P'):
            try:
                element = elm.PFet(bulk=True, scale=scale).at((x, y))  # [MODIFIED]
            except AttributeError:
                print("Error: schemdraw does not have 'PFet' element. Please verify the element name.")
                return
        else:
            try:
                element = elm.NFet(bulk=True, scale=scale).at((x, y))  # [MODIFIED]
            except AttributeError:
                print("Error: schemdraw does not have 'NFet' element. Please verify the element name.")
                return
    else:
        ...
        if comp_class is None:
            ...
            return
        # [MODIFIED] Apply per-component scale
        element = comp_class(**node_info.get('parameters', {}), scale=scale).at((x, y))
...
def draw_circuit(G, components_element, xml_root, xml_file, initial_comp_node_mapping, max_attempts=100,
    ...
    scaling_ratios = {}  # Read scale attribute
    ...
    if components_elem is not None:
        for comp in components_elem:
            ...
            scale_val = float(comp.attrib.get('scale', str(default_scaling_ratios.get(comp.attrib['type'], 1.0))))  # Read scale attribute
            directions_in_xml[cid] = dir_val
            flips_in_xml[cid] = flip_val
            scaling_ratios[cid] = scale_val  # Read scale
            if 'x' in comp.attrib and 'y' in comp.attrib:
                positions_in_xml[cid] = (float(comp.attrib['x']), float(comp.attrib['y']))
            print(
                f"[DEBUG] Component '{cid}': direction='{dir_val}', flip='{flip_val}', scale='{scale_val}', position={positions_in_xml.get(cid, 'Not set')}")
    ...
    if auto == 1:
        ...
        while attempt < max_attempts:
            ...
            for n in G.nodes:
                ...
                scales[n] = scaling_ratios.get(n, default_scaling_ratios.get(G.nodes[n]['type'], 1.0))  # Assign scale
            ...
            for node in G.nodes:
                ...
                scale = scales.get(node, 1.0)  # Assign scale
                print(
        ...
        if not success:
            ...
            if last_pos is not None and last_all_wires is not None:
                ...
                for node in G.nodes:
                    ...
                    scale = scaling_ratios.get(node, 1.0)  # Assign scale
                    print(
        ...
        for node in G.nodes:
            ...
            scale = scaling_ratios.get(node, default_scaling_ratios.get(G.nodes[node]['type'], 1.0))  # Assign scale
