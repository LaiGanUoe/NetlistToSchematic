...
def draw_circuit(G, components_element, xml_root, xml_file, initial_comp_node_mapping, max_attempts=100,
    ...
    if components_elem is not None:
        for comp in components_elem:
            ...
            flip_val = comp.attrib.get('flip', 'none')  # 读取 flip 属性
    ...
    if auto == 1:
        ...
        while attempt < max_attempts:
            ...
            # 使用 initial_comp_node_mapping 的深拷贝
            ...
            if any_red_line or overlapping:
                ...
                update_xml_positions_and_directions(xml_root, pos, directions, flips)  # [NEW CODE] 更新 flip
        ...
        for node in G.nodes:
            ...
            print(f"[DEBUG] Drawing node/component '{node}' at ({x}, {y}) with direction '{direction}' and flip '{flip_val}'")
            draw_component_or_node(d, elements, pins, node, node_info, x, y, direction, component_boxes, G,
                                   flip=flip_val)
        ...
        # 更新 XML 中的位置和方向
        update_xml_positions_and_directions(xml_root, pos, directions, flips)
        tree = ET.ElementTree(xml_root)
        tree.write(xml_file)
        d.draw()
